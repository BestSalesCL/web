    ...
    const fetchSessionDetails = async () => {
      if (sessionId && priceId) {
        try {
          ...
          const amount = parseFloat(priceId || "0");

          // Send data to Facebook Conversion API
          const fbResponse = await fetch("/api/facebook-purchase", {
            method: "POST",
            headers: {
          ...
          if (!fbResponse.ok) {
            console.error("Failed to send purchase event to Facebook");
          } else {
            const data = await fbResponse.json();
            console.log("Purchase event sent successfully to Facebook", data);
          }
        } catch (error) {
    ...
    <section className="flex-center-col mt-[60px] size-full overflow-hidden px-6 pb-[64px] pt-8 sm:px-[40px] md:px-[100px] md:pb-[96px]">
      <div className="flex h-auto max-w-[800px] flex-col items-center justify-start gap-8 rounded-[24px] bg-primary_200 px-6 py-8 sm:px-8 md:py-10">
        <h1 className="h2-small md:h1-small w-full text-center text-background_color">
          Thank You for Choosing BestLevel!
        </h1>
        <div className="flex-start-col w-full gap-6">
          <p className="text-20 text-text_900">
            We’re excited to have you with us! Your journey to reaching your
            full potential starts now. Get ready to crush your goals and reach
            your BestLevel. Let’s do this together! 🚀💪
          </p>
          {priceId === "30" && (
            <div className="flex-start-col gap-[4px]">
              <p className="subtitle text-background_color">
                To start enjoying your Hobby Plan:
              </p>
              <p className="text-20 text-text_900">
                Check your email inbox, follow the instructions, and begin your
          ...
          {priceId === "300" && (
            <div className="flex-start-col gap-[4px]">
              <p className="subtitle text-background_color">
                To start enjoying your Performance Plan:
              </p>
              <p className="text-20 text-text_900">
                Check your email inbox, follow the instructions, and begin your
                journey to your full potential.
